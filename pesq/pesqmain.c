...
double pesq_main(char* reference, char* degraded, int rate, char* mode);
void usage (void);
void pesqMeasure (SIGNAL_INFO * ref_info, SIGNAL_INFO * deg_info,
...
double pesq_main(char* reference, char* degraded, int rate, char* mode)
{
    ...
    pesqMeasure (&ref_info, &deg_info, &err_info, &Error_Flag, &Error_Type);
    ...
    ERROR_INFO * err_info, long * Error_Flag, char ** Error_Type)
{
    ...
    if ((*Error_Flag) == 0)
    {
        printf ("\nReading reference file %s...", ref_info-> path_name);
